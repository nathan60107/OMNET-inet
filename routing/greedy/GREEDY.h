...
#include "inet/routing/greedy/GREEDY_m.h"
 ...
 *
 * For more information on the routing algorithm, see the GREEDY paper
...
// KLUDGE: the GREEDY packet is now used to wrap the content of network datagrams
// KLUDGE: we should rather add these fields as header extensions
class INET_API GREEDY : public cSimpleModule, public ILifecycle, public cListener, public INetfilter::IHook
{
  private:
    // GREEDY parameters
    GREEDYPlanarizationMode planarizationMode = (GREEDYPlanarizationMode)-1;
  ...
  private:
    ...
    // handling packets
    GREEDYOption *createGreedyOption(L3Address destination, cPacket *content);
    int computeOptionLength(GREEDYOption *greedyOption);
    void setGreedyOptionOnNetworkDatagram(INetworkDatagram *datagram);
    void removeGreedyOptionFromNetworkDatagram(INetworkDatagram *datagram);

    // returns nullptr if not found
    GREEDYOption *findGreedyOptionInNetworkDatagram(INetworkDatagram *datagram);
    const GREEDYOption *findGreedyOptionFromNetworkDatagram(INetworkDatagram *datagram) const { return const_cast<GREEDY *>(this)->findGreedyOptionFromNetworkDatagram(datagram); }

    // throws an error when not found
    GREEDYOption *getGreedyOptionFromNetworkDatagram(INetworkDatagram *datagram);
    const GREEDYOption *getGreedyOptionFromNetworkDatagram(INetworkDatagram *datagram) const { return const_cast<GREEDY *>(this)->getGreedyOptionFromNetworkDatagram(datagram); }
